...
export class JobsService {
  async createJob(userId: string, input: CreateJobInput) {
    ...
    await publishToQueue(QUEUES.JOB, {
      jobId: job.id, userId,
      keyword: job.keyword, city: job.city, country: job.country,
      radius: job.radius, maxResults: job.maxResults,
  ...
  async getJobById(jobId: string, userId: string) {
    ...
    const liveProgress = await cache.get(cache.jobProgressKey(jobId))
  ...
  async deleteJob(jobId: string, userId: string) {
    ...
    await cache.del(cache.jobProgressKey(jobId))
  }
